"""JSON emitter for rvi_robinhood -> dashboard/data/rvi_robinhood.json.
Reuses the VCX engine modules (premium, scenarios, nav_markto)."""
...
from config import RviRobinhood as CFG, DASHBOARD_DATA_DIR
...
from situations.rvi_robinhood.ingest import nav_log

_SIT = os.path.join(_REPO_ROOT, "situations", "rvi_robinhood")
_PROCESSED = os.path.join(_SIT, "data", "processed")

DISCLAIMER = ("Analysis, not investment advice. RVI is a closed-end fund: market price can detach from "
              "NAV. NAV is sponsor-published and carried forward. RVI's NPORT names holdings directly "
              "(no SPVs), but its headline OpenAI stake was bought 2026-04-17 — AFTER the 3/31 filing — so "
              "OpenAI look-through is sponsor-disclosed until the next NPORT. The fund is ~53% cash. Every "
              "figure is labeled with a confidence level. No datapoint is fabricated.")
...
def build_payload():
    ...
    cash_pct = None
    if nport:
        for h in nport[-1].get("top_holdings", []):
            nm = h["name"].lower()
            if "government" in nm or "treasury" in nm or "money market" in nm:
                cash_pct = h["pctVal"] / 100.0
                break
